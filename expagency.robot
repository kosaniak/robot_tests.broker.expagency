...
Отримати інформацію із тендера
    ...
    Run keyword    expagency.Перейти на сторінку тендера  ${username}  ${tender_uaid}
    ${return_value}=  Run Keyword  expagency.Отримати інформацію про ${fieldname}
    [return]  ${return_value}
...
Отримати інформацію про contracts[-1].datePaid
    [Arguments]    @{ARGUMENTS}
    Run keyword    expagency.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[1]}
