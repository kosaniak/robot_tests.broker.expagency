...
def convert_datetime_to_expagency_format(isodate):
    iso_dt = parse_date(isodate)
    day_string = iso_dt.strftime("%d/%m/%Y %H:%M")
    return day_string


def convert_string_from_dict_expagency(string):
    return {
        u"грн": u"UAH",
        u"True": u"1",
        u"False": u"0",
...
def adapt_view_data(value, field_name):
    ...
    return convert_string_from_dict_expagency(value)
...
def get_upload_file_path():
    return os.path.join(os.getcwd(), 'src/robot_tests.broker.expagency/testFileForUpload.txt')


def expagency_download_file(url, file_name, output_dir):
